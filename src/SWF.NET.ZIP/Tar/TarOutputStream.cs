...
public class TarOutputStream : Stream
{
	...
	public override bool CanSeek =>
        outputStream.CanSeek; // -jr- Should be false?
	...
	public void PutNextEntry(TarEntry entry)
    {
        if (entry.TarHeader.name.Length > TarHeader.NAMELEN)
        {
            ...
            {
                TarHeader.GetNameBytes(entry.TarHeader.name, nameCharIndex, blockBuf, 0, TarBuffer.BlockSize);
	...
	public void CloseEntry()
    {
        if (assemLen > 0)
        {
            for (var i = assemLen; i < assemBuf.Length; ++i) assemBuf[i] = 0;
        ...
        if (currBytes < currSize) throw new IOException("entry closed at '" + currBytes + "' before the '" + currSize + "' bytes specified in the header were written");
    }
    ...
    {
        if (wBuf == null) throw new ArgumentNullException("TarOutputStream.Write buffer null");

        if (currBytes + numToWrite > currSize) throw new ArgumentOutOfRangeException("request to write '" + numToWrite + "' bytes exceeds size in header of '" + currSize + "' bytes");
    ...
    {
        for (var i = 0; i < blockBuf.Length; ++i) blockBuf[i] = 0;
        buffer.WriteBlock(blockBuf);
    }
}
