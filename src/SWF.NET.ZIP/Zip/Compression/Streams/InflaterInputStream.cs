...
using SWF.NET.ZIP.GZip;

namespace SWF.NET.ZIP.Zip.Compression.Streams;
...
public class InflaterInputStream : Stream
{
    ...
    /// </remarks>
    protected int BufferReadSize { get; set; } = 0;
    ...
    {
        if (baseInputStream == null) throw new ArgumentNullException("InflaterInputStream baseInputStream is null");

        if (inflater == null) throw new ArgumentNullException("InflaterInputStream Inflater is null");
...
*/
    ...
    protected void FillInputBuffer()
    {
        if (BufferReadSize <= 0)
            ...
            len = baseInputStream.Read(buf, 0, BufferReadSize);
    }
    ...
    protected void Fill()
    {
        ...
        if (keys != null) DecryptBlock(buf, 0, len);
    ...
    {
        var nread = Read(onebytebuffer, 0, 1); // read one byte
    ...
    protected void InitializePassword(string password)
    {
        ...
        for (var i = 0; i < password.Length; ++i) UpdateKeys((byte)password[i]);
    }
