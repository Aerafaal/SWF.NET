...
public class StreamManipulator
{
    ...
    public int AvailableBytes =>
        window_end - window_start + (AvailableBits >> 3);
    ...
    public int PeekBits(int n)
    {
        if (AvailableBits < n)
        {
            if (window_start == window_end) return -1; // ok
    ...
    {
        if (length < 0) throw new ArgumentOutOfRangeException("length", "negative");
        ...
        var avail = window_end - window_start;
    ...
    /// </summary>
    public void Reset() =>
        buffer = (uint)(window_start = window_end = AvailableBits = 0);
    ...
    public void SetInput(byte[] buf, int off, int len)
    {
        ...
        */
        ...
        {
            /* We always want an even number of bytes in input, see peekBits */
            buffer |= (uint)((buf[off++] & 0xff) << AvailableBits);
            AvailableBits += 8;
        }
