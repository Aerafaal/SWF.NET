...
public class DeflaterEngine : DeflaterConstants
{
    ...
    private bool FindLongestMatch(int curMatch)
    {
        ...
        */
        ...
        if (DEBUGGING && strstart > 2 * WSIZE - MIN_LOOKAHEAD) throw new InvalidOperationException("need lookahead");

        do
        {
            if (DEBUGGING && curMatch >= strstart) throw new InvalidOperationException("future match");
    ...
    {
        if (DEBUGGING && strstart != 1) throw new InvalidOperationException("strstart not 1");
    ...
    private bool DeflateStored(bool flush, bool finish)
    {
        ...
        var storedLen = strstart - blockStart;
    ...
    private bool DeflateSlow(bool flush, bool finish)
    {
        ...
        while (lookahead >= MIN_LOOKAHEAD || flush)
        {
            if (lookahead == 0)
            {
                ...
                /* We are flushing everything */
                if (DEBUGGING && !flush) throw new Exception("Not flushing, but no lookahead");
                huffman.FlushBlock(
                    window,
                    blockStart,
                    strstart - blockStart,
