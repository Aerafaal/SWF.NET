...
public class ZipOutputStream : DeflaterOutputStream
{
    ...
    public void SetComment(string comment)
    {
        ...
        if (commentBytes.Length > 0xffff) throw new ArgumentOutOfRangeException("comment", "Comment too long.");
    ...
    public void PutNextEntry(ZipEntry entry)
    {
        ...
        if (entries.Count >= 0xffff) throw new ZipException("Too many entries for Zip file");
        ...
        if (method == CompressionMethod.Stored)
        {
            if (entry.CompressedSize >= 0)
            {
                ...
                else if (entry.Size != entry.CompressedSize) throw new ZipException("Method STORED, but compressed size != size");
            }
            else
            {
                if (entry.Size >= 0) entry.CompressedSize = entry.Size;
            }
        ...
        if (method == CompressionMethod.Deflated)
        {
            ...
            else if (entry.CompressedSize < 0 || entry.Size < 0 || entry.Crc < 0) headerInfoAvailable = false;
        }
        ...
        {
            if (patchEntryHeader) headerPatchPos = baseOutputStream.Position;
            WriteLeInt(0); // Crc
            WriteLeInt(0); // Compressed size
            WriteLeInt(0); // Uncompressed size
        }

        var name = ZipConstants.ConvertToArray(entry.Name);

        if (name.Length > 0xFFFF) throw new ZipException("Entry name too long.");

        var extra = entry.ExtraData;
        if (extra == null) extra = new byte[0];

        if (extra.Length > 0xFFFF) throw new ZipException("Extra data too long.");
    ...
    public void CloseEntry()
    {
        ...
        if (curMethod == CompressionMethod.Deflated) base.Finish();

        var csize = curMethod == CompressionMethod.Deflated ? def.TotalOut : size;

        if (curEntry.Size < 0)
            curEntry.Size = size;
        else if (curEntry.Size != size) throw new ZipException("size was " + size + ", but I expected " + curEntry.Size);

        if (curEntry.CompressedSize < 0)
            curEntry.CompressedSize = csize;
        else if (curEntry.CompressedSize != csize) throw new ZipException("compressed size was " + csize + ", but I expected " + curEntry.CompressedSize);

        if (curEntry.Crc < 0)
            curEntry.Crc = crc.Value;
        else if (curEntry.Crc != crc.Value) throw new ZipException("crc was " + crc.Value + ", but I expected " + curEntry.Crc);

        offset += csize;

        if (offset > 0xffffffff) throw new ZipException("Maximum Zip file size exceeded");

        if (curEntry.IsCrypted) curEntry.CompressedSize += ZipConstants.CRYPTO_HEADER_SIZE;

        // Patch the header if possible
        if (patchEntryHeader)
        {
            var curPos = baseOutputStream.Position;
    ...
    public override void Write(byte[] b, int off, int len)
    {
        ...
        if (size > 0xffffffff || size < 0) throw new ZipException("Maximum entry size exceeded");
    ...
    public override void Finish()
    {
        ...
        foreach (ZipEntry entry in entries)
        {
            ...
            var entryComment = entry.Comment != null ? ZipConstants.ConvertToArray(entry.Comment) : new byte[0];
