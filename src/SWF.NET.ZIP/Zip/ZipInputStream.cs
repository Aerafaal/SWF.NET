...
public class ZipInputStream : InflaterInputStream
{
    ...
    public bool CanDecompressEntry =>
        entry != null && entry.Version <= ZipConstants.VERSION_MADE_BY;
    ...
    private void ReadFully(byte[] outBuf)
    {
        ...
        while (len > 0)
        {
            var count = ReadBuf(outBuf, off, len);
            if (count <= 0) throw new ZipException("Unexpected EOF");
            off += count;
            len -= count;
        }
    }
    ...
    public ZipEntry GetNextEntry()
    {
        ...
        if (avail <= 0) FillBuf(ZipConstants.LOCHDR);

        var header = ReadLeInt();
        ...
        if (header == ZipConstants.SPANTEMPSIG || header == ZipConstants.SPANNINGSIG) header = ReadLeInt();

        if (header != ZipConstants.LOCSIG) throw new ZipException("Wrong Local header signature: 0x" + string.Format("{0:X}", header));

        var versionRequiredToExtract = (short)ReadLeShort();

        flags = ReadLeShort();
        method = ReadLeShort();
        var dostime = (uint)ReadLeInt();
        var crc2 = ReadLeInt();
        csize = ReadLeInt();
        size = ReadLeInt();
        ...
        if (method != (int)CompressionMethod.Stored && method != (int)CompressionMethod.Deflated) throw new ZipException("unknown compression method " + method);
        ...
        if (versionRequiredToExtract > 20) throw new ZipException("Libray cannot extract this entry version required (" + versionRequiredToExtract + ")");
        ...
        if (isCrypted)
        {
            ...
            if ((flags & 8) == 0)
            {
                if (cryptbuffer[ZipConstants.CRYPTO_HEADER_SIZE - 1] != (byte)(crc2 >> 24)) throw new ZipException("Invalid password");
            }
            else
            {
                if (cryptbuffer[ZipConstants.CRYPTO_HEADER_SIZE - 1] != (byte)((dostime >> 8) & 0xff)) throw new ZipException("Invalid password");
            }
        ...
        if (method == (int)CompressionMethod.Deflated && avail > 0)
        {
            ...
            len = (int)avail;
            avail = 0;
            if (isCrypted) DecryptBlock(buf, 0, Math.Min((int)csize, len));
    ...
    {
        if (ReadLeInt() != ZipConstants.EXTSIG) throw new ZipException("Data descriptor signature not found");
    ...
    public void CloseEntry()
    {
        ...
        else
        {
            ...
            while (csize != 0)
            {
                var skipped = (int)Skip(csize & 0xFFFFFFFFL);
    ...
    public override int Read(byte[] b, int off, int len)
    {
        ...
        switch (method)
        {
            ...
            case (int)CompressionMethod.Stored:
                if (len > csize && csize >= 0) len = (int)csize;
                ...
                {
                    if (len < 0) throw new ZipException("EOF in stored block");
                }

                // cipher text needs decrypting
                if (cryptbuffer != null) DecryptBlock(b, off, len);
        ...
        if (finished)
        {
            ...
            if ((crc.Value & 0xFFFFFFFFL) != entry.Crc && entry.Crc != -1) throw new ZipException("CRC mismatch");
            crc.Reset();
            entry = null;
        }
