...
public class Swf : IBaseTagContainer
{
    ...
    /// </summary>
    private readonly BaseTagCollection tagList;
    ...
    ///     Swf header.
    /// </summary>
    private readonly SwfHeader header;
    ...
    public Swf()
    {
        ...
        tagList.TagAdded += tagList_TagAdded;
        Init();
    }
    ...
    ///     <see cref="DefineTag">DefineTag</see>
    ///     extended tags.
    ...
    public int ByteCount
    {
        ...
        {
            var len = 0;
            foreach (BaseTag tag in tagList) len += tag.Data.Length;
            var size = 12 + header.Size.GetSizeOf() + len;
            return size;
        }
    }
    ...
    public void InjectTags(BaseTagCollection values)
    {
        ...
        for (var i = 0; enums.MoveNext(); i++)
        {
            var baseTag = (BaseTag)enums.Current;
            Add(baseTag);
        }
    }
    ...
    public ushort GetNewDefineId()
    {
        ...
        while (!available)
        {
            i++;
            available = true;
            var keys = dictionnary.Keys.GetEnumerator();
            while (keys.MoveNext())
            {
                var key = (ushort)keys.Current;
                if (key == i)
                {
                    available = false;
                }
