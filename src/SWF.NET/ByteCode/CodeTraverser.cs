...
///     by stepping through action lists and following branches. In opposite to <see cref="CodeWalker" />
...
{
	/// <summary>
	///     ArrayList of action objects derived from <see cref="BaseAction" />.
	/// </summary>
    ...
    private void traverse(int index, IActionExaminer examiner)
    {
        while (index < actionRec.Count)
        {
            ...
            if (a is ActionIf)
            {
                ...
                if (!visitedLabels.Contains(ai.LabelId)) traverse((int)labelIndexTable[ai.LabelId], examiner.Clone());
            }
