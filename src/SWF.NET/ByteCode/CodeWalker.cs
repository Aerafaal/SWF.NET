...
///     by stepping through code and following branches. In opposite to <see cref="CodeTraverser" />
...
{
    private readonly ArrayList actionRec;
    private readonly SortedList labelIndexTable;
    ...
    public CodeWalker(ArrayList actions)
    {
        ...
        {
            var lbl = actions[i] as ActionLabel;
            if (lbl != null) labelIndexTable[lbl.LabelId] = i;
        }
    }
    ...
    private void Walk(int index, IActionExaminer examiner, ArrayList visitedLabels)
    {
        ...
        {
            var a = (BaseAction)actionRec[index];

            if (a is ActionLabel)
            {
                var l = a as ActionLabel;
            ...
            if (a is ActionJump)
            {
                var j = a as ActionJump;
                index = (int)labelIndexTable[j.LabelId] - 1;
            }

            if (a is ActionIf)
            {
                var ai = a as ActionIf;
                if (!visitedLabels.Contains(ai.LabelId)) Walk((int)labelIndexTable[ai.LabelId], examiner.Clone(), (ArrayList)visitedLabels.Clone());
            }
    ...
    {
        var visitedLabels = new ArrayList();
