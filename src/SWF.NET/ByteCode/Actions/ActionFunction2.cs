...
public class ActionDefineFunction2 : MultiByteAction
{
    ...
    /// <summary>
    ///     <see cref="RegParamPair">RegParamPair</see> of expected parameters
    /// </summary>
    ...
    public override int ByteCount
    {
        get
        {
            var count = 3 + Name.Length + 1 + 2 + 1 + 2 + 2 + InnerByteCount;
            for (var i = 0; i < ParameterList.Length; i++)
            {
                var r = ParameterList[i];
    ...
    {
        var s = new string[ParameterList.Length];
        ...
        var s2 = new string[ActionRecord.Count];
        for (var i = 0; i < ActionRecord.Count; i++)
        {
    ...
    public struct RegParamPair
    {
        ...
        /// </summary>
        public override string ToString() =>
            string.Format("r:{0}='{1}'", Register, Parameter);
    }
    ...
    public struct VariableFlagSet
    {
        ...
        {
            if (PreloadSuper) SuppressSuper = false;
            if (PreloadArguments) SuppressArguments = false;
            if (PreloadThis) SuppressThis = false;
        }
