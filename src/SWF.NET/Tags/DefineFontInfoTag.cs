...
public class DefineFontInfoTag : BaseTag, DefineTag
{
    ...
    /// </summary>
    public bool FontFlagsAINSI { get; set; }
    ...
    public override void ReadData(byte version, BufferedBinaryReader binaryReader)
    {
        ...
        var codeTableLenght = rh.TagLength - 4 - fontNameLen;

        if (!fontFlagsWildCodes)
        {
            codeTable = new uint[codeTableLenght];
            for (var i = 0; i < codeTableLenght; i++)
                codeTable[i] = binaryReader.ReadByte();
        }
        else
        {
