...
public class ClipActionRec
{
    ...
    /// </summary>
    public byte[] ActionRecord { get; set; }
    ...
    public bool ReadData(BinaryReader br, byte version)
    {
        ...
        if (version >= 6)
        {
            ...
            // swf events
            var f6Events = header[2];
            var hasKeyPress = (f6Events & 0x02) != 0;
            var hasConstruct = (f6Events & 0x04) != 0; // Flash 7 +
            ...
            ActionRecord = br.ReadBytes(size);
        }
        else
        {
    ...
    {
        var offset = ActionRecord.Length;
        if (keyCode > -1) offset++;
