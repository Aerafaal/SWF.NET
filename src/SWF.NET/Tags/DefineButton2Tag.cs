...
public class DefineButton2Tag : BaseTag, DefineTag
{
    ...
    /// </summary>
    /// <value></value>
    public ButtonRecordCollection Characters { get; private set; }
    ...
    public override void ReadData(byte version, BufferedBinaryReader binaryReader)
    {
        ...
        var curr = startPos + actionOffset;

        actions = new ButtonCondactionCollection();
        var lastCondAction = false;
        if (actionOffset == 0)
            lastCondAction = true;

        while (!lastCondAction)
        {
            var readedBytes = binaryReader.BaseStream.Position - startPosition;
            var condActionSize = binaryReader.ReadUInt16();
            if (condActionSize == 0)
            {
                lastCondAction = true;
            ...
            var buttCond = new ButtonCondaction();
    ...
    protected int GetSizeOf()
    {
        ...
        if (Characters != null)
        {
            var butts = Characters.GetEnumerator();
        ...
        if (actions != null)
        {
            var buttConds = actions.GetEnumerator();
    ...
    public override void UpdateData(byte version)
    {
        ...
        if (actions != null)
            ...
            {
                var buttCon = actions[i];
                if (i == actions.Count - 1)
                    w.Write((ushort)0);
                else
                {
                    var size = buttCon.GetSizeOf();
                    w.Write((ushort)size);
                }
    ...
    public override void Serialize(XmlWriter writer)
    {
        ...
        writer.WriteAttributeString("ButtonId", CharacterId.ToString());
