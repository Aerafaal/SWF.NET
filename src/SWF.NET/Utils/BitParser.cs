...
internal class BitParser
{
	...
	/// </remarks>
	private static readonly int[] bitValueList = new int[32];
	...
	static BitParser()
    {
        ...
        for (var i = 1; i < 32; i++) bitValueList[i] = BitVector32.CreateMask(bitValueList[i - 1]);
    }
    ...
    {
        var byteCount = bitArr.Length / 8;
        for (var i = 0; i < byteCount; i++)
        for (var j = 0; j < 4; j++)
        {
            var temp = bitArr[i * 8 + 7 - j];
    ...
    {
        var ba = new BitArray(byteSequence);
        BytewiseReverse(ba);
        return ba;
    }
    ...
    {
        var bitVec = new BitVector32(0);
        for (var i = 0; i < length; i++) bitVec[bitValueList[i]] = bitArr[index + length - i - 1];
        var sign = bitArr[index];
        for (var i = length; i < 32; i++) bitVec[bitValueList[i]] = sign;
        return bitVec.Data;
    }
	...
	public static int ReadInt32(BitArray bitArr) =>
        ReadInt32(bitArr, 0, bitArr.Length);
	...
	public static int ReadUInt32(BitArray bitArr) =>
        ReadUInt32(bitArr, 0, bitArr.Length);
