...
public class BufferedBinaryReader : BinaryReader
{
    ...
    public byte[] PeekBytes(uint numToPeek)
    {
        if (numToPeek <= 0)
            return null;
        var res = new byte[numToPeek];
        for (var i = 0; i < numToPeek; i++)
            res[i] = PeekByte();
        return res;
    }
    ...
    {
        var val = ReadUInt16();
        base.BaseStream.Seek(-2, SeekOrigin.Current);
        return val;
    }
    ...
    {
        var val = ReadUInt32();
        base.BaseStream.Seek(-4, SeekOrigin.Current);
        return val;
    }
    ...
    public float ReadFloat(uint numBits)
    {
        ...
        value = ReadSBits(numBits) / divisor;
        return value;
    }
    ...
    public float ReadFloatWord(uint mantissaSize, uint fractionSize)
    {
        ...
        float value = 0;
        float divisor = 1 << (int)fractionSize;

        fraction = (int)ReadUBits(fractionSize);
        mantissa = ReadSBits(mantissaSize);
    ...
    {
        var v = (int)ReadUBits(bits);

        if ((v & (1L << (int)(bits - 1))) > 0) v |= -1 << (int)bits;
